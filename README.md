...
| [Reading from S3](./reading-from-s3) | Read Parquet straight from Amazon S3 (or any S3-compatible store) — configure an `S3Source`, fetch byte ranges instead of whole objects, and open a whole bucket as one dataset. Runs against a local MinIO server in Docker. |
