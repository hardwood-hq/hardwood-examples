...
| [Geospatial](./geospatial) | Read a GEOMETRY column and push a bounding-box filter down to the row-group level — recognize the logical type, inspect per-row-group bounding boxes, skip disjoint groups, and decode WKB points with JTS.<br/>**Concepts:** `LogicalType.GeometryType`/`GeographyType`, `GeospatialStatistics`, `BoundingBox`, `FilterPredicate.intersects`, WKB decode via `jts-core` |
