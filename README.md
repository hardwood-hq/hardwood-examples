...
- *(Optional)* **Docker**, only for examples that need it (e.g. the S3 example starts a
  containerized S3 server) or if you prefer running an example in a container.
...
| [Avro Integration](./avro-integration) | Materialize Parquet rows as Avro `GenericRecord` — read the derived Avro schema, stream records the way Kafka/Spark consumers expect, and push filters and head/tail limits into the reader. |
